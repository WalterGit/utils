...
ADDITIONAL_TEXT="import type { LinksFunction } from \"@remix-run/node\";\\
...
if [ -f "$FILE" ]; then
    # Adiciona o texto antes da linha que começa com 'export function Layout'
    sed -i '' $'/^export function Layout/{i\\
'"$ADDITIONAL_TEXT"'
    }' $FILE
